...
class Pretrain_Graph(LightningModule):
    def __init__(self,cfg: Dict[str,Any], stage='pretrain'):
        ...
        if stage == 'inference':
            self.vocab = pickle.load(open("/root/jupyter/DAG_Transformer/DAG_Transformer/dataset/syn_dag_from_qb/data/selfies_vocab.pkl", 'rb'))
        else:
            self.vocab = pickle.load(open("/root/jupyter/DAG_Transformer/DAG_Transformer/dataset/syn_dag_from_qb/data/selfies_vocab.pkl", 'rb'))
    ...
    def generate_reaction_fp(self, reaction_rxn, device):
        ...
        return torch.cat([torch.sum(graph_input_encoding[:, 1:, :], dim=1), 
                          torch.sum(graph_reactants_encoding[:, 1:, :], dim=1)], dim=1)
    ...
    @torch.no_grad()
    def generate_reaction_pair(self, reaction_rxn, device):
        input_mol = Chem.MolFromSmiles(reaction_rxn[0][0][0])
        reactant_mol = Chem.MolFromSmiles('.'.join([reaction_rxn[0][1][i][0] for i in range(len(reaction_rxn[0][1]))]))
        reagent_list = reaction_rxn[1].split('.')
        input_fp, input_item = self.generate_reactant_fp(input_mol, device)
        ...
        #reactant_fp = self.reactants_head(reactant_fp[:, 0, :]) + self.reagent_head(reagent_fp)
        return torch.cat([input_fp[:, 0, :], reactant_fp[:, 0, :], reagent_fp], dim=-1)

    @torch.no_grad()
    def generate_reaction_fp_yield(self, reactant, reagent, product, device):
        reactant = Chem.MolFromSmiles(reactant)
        reagent = reagent.split('.')
        reactant_fp, reactant_item = self.generate_reactant_fp(reactant, device)
        reagent_fp = self.generate_reagent_fp(reagent, device)
        product_fp = self.generate_product_fp(product, device)
        return torch.cat([reactant_fp[:, 0, :], reagent_fp, product_fp], dim=1)
    
    @torch.no_grad()
    def generate_reaction_fp_SN2(self, reactant_smi, product_smi, device):
        reactant = Chem.MolFromSmiles(reactant_smi.split('.')[0])
        reactant_sub = Chem.MolFromSmiles(reactant_smi.split('.')[1])
        product = Chem.MolFromSmiles(product_smi.split('.')[0])
        product_sub = Chem.MolFromSmiles(product_smi.split('.')[1])
        reactant_fp, reactant_item = self.generate_reactant_fp(reactant, device)
        reactant_sub_fp, reactant_sub_item = self.generate_reactant_fp(reactant_sub, device)
        product_fp, product_item = self.generate_product_fp(product, device)
        product_sub_fp, product_sub_item = self.generate_product_fp(product_sub, device)
        return torch.cat([reactant_fp[:, 0, :],reactant_sub_fp[:, 0, :], product_fp[:, 0, :], product_sub_fp[:, 0, :]], dim=1)
